  ...
  setup(__props, { emit }) {
    ...
    const handleNavigate = (item, num) => {
      common_vendor.index.navigateTo({
        url: `/subPages/order/cancel?id=${item.id}&type=${num}&tradingOrderNo=${item.tradingOrderNo || item.orderId || ''}`
      });
    };
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        d: common_vendor.f(__props.itemData, (item, index, i0) => {
          return common_vendor.e({
            a: item.image || "/static/images/default-service.png",
            b: common_vendor.t(item.serviceName || item.amount),
            c: item.status === 0
          }, item.status === 0 ? {} : {}, {
            d: item.status === 1
          }, item.status === 1 ? {} : {}, {
            ...
            i: common_vendor.t(item.userName || (item.elderVo && item.elderVo.name)),
            j: common_vendor.t(item.createTime ? item.createTime.substring(0, 10) : (item.estimatedArrivalTime ? common_vendor.unref(utils_index.getNow)(item.estimatedArrivalTime) : '')),
            k: common_vendor.t(item.createTime ? item.createTime.substring(11, 19) : (item.estimatedArrivalTime ? common_vendor.unref(utils_index.getTime)(item.estimatedArrivalTime) : '')),
            l: common_vendor.t(common_vendor.unref(utils_index.decimalsReplenish)(item.amount)),
            //m: common_vendor.o(($event) => handleDetail(item.id), index),//yangeit禁止
            n: item.status === 0
          }, item.status === 0 ? {
            o: common_vendor.o(($event) => handleCancel(item), index),
            p: common_vendor.o(($event) => handlePay(item), index)
          } : {}, {
            q: item.status === 1 && (!item.refundStatus && !item.refundRecordVo || item.refundStatus === 3 || (item.refundRecordVo && item.refundRecordVo.refundStatus === 3))
          }, item.status === 1 && (!item.refundStatus && !item.refundRecordVo || item.refundStatus === 3 || (item.refundRecordVo && item.refundRecordVo.refundStatus === 3)) ? {
